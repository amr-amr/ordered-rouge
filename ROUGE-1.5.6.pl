...
# Version:     ROUGE v1.5.6
# Date:        05/22/2019,05/26/2005,05/19/2005,04/26/2005,04/03/2005,10/28/2004,10/25/2004,10/21/2004
# Author:      Andrei Mircea Romascanu, Chin-Yew Lin
...
use Getopt::Std;
#-------------------------------------------------------------------------------------
use vars qw($opt_a $opt_b $opt_c $opt_d $opt_e $opt_f $opt_h $opt_H $opt_m $opt_n $opt_o $opt_p $opt_s $opt_t $opt_l $opt_v $opt_w $opt_2 $opt_u $opt_x $opt_U $opt_3 $opt_M $opt_z);
my $usageFull="$0\n         [-a (evaluate all systems)]
         ...
         [-n max-ngram]
         [-o order weight (default 0.95)]                                                                        # 1.5.6
         [-s (remove stopwords)]
...
".
  ...
  "  -o: Order weight used for computing ROUGE-ON (default: 0.95).\n".                                           # 1.5.6
...
my $usage="$0\n         [-a (evaluate all systems)]
         ...
         <ROUGE-eval-config-file> [<systemID>]
";
getopts('ahHb:c:de:f:l:mMn:o:p:st:r:2:3:w:uUvxz:');
my $systemID;
...
if (defined($opt_o)) {                                                                                        # 1.5.6
  $ROUGEParam{"ORDERWEIGHT"}=$opt_o;
}
else {
  $ROUGEParam{"ORDERWEIGHT"}=0.95
}
    ...
    if(defined($opt_n)) {
      ...
      for($n=1;$n<=$opt_n;$n++) {
        ...
        foreach $e (@ROUGEEvalIDs) {
          ...
          $ROUGEParam{"NSIZE"}=$n;
          &computeROUGEX("ON",\%ROUGEScores,$e,$ROUGEEvals{$e},$peerID,\%ROUGEParam);
        }
        # compute averages
        %ROUGEAverages=();
        &computeAverages(\%ROUGEScores,\%ROUGEAverages,$opt_t);
        &printResults($peerID,\%ROUGEAverages,\%ROUGEScores,"ROUGE-O$n",$opt_c,$opt_t,$opt_d);
      }
    }
...
sub computeROUGEX {
  ...
  my $orderWeight;        # orderweight for ROUGE-ON                                                      # 1.5.6
  ...
  $orderWeight=$ROUGEParam->{"ORDERWEIGHT"};                                                              # 1.5.6
  ...
  # evaluate peer
  {
    ...
    elsif($metric eq "ON") {                                                                              # 1.5.6
      &computeONGramScore(\@modelPaths,$peerPath,\@results,$NSIZE,$orderWeight,$lengthLimit,$byteLimit,$inputFormat,$scoreMode,$alpha);
    }
    elsif($metric eq "L") {
...
sub computeONGramScore {
  ...
  $peerText="";
  &readText($peerPath,\$peerText,$inputFormat,$lengthLimit,$byteLimit);
  &createONGram($peerText,\%peer_grams,$NSIZE);                                                           # 1.5.6
  if($debug) {
    print "***P $peerPath\n";
    if(defined($peerText)) {
      print "$peerText\n";
  ...
  foreach $modelPath (@$modelPaths) {
    ...
    $modelText="";
    &readText($modelPath,\$modelText,$inputFormat,$lengthLimit,$byteLimit);
    &createONGram($modelText,\%model_grams,$NSIZE);                                                       # 1.5.6
    if($debug) {
      if(defined($modelText)) {
	print "$modelText\n";
	print join("|",%model_grams),"\n";
    ...
    #------------------------------------------------
    # compute ngram score
    &ongramScore(\%model_grams,\%peer_grams,\$gramHit,\$gramScore,$orderWeight);                          # 1.5.6
    # collect hit and count for each models
    # This will effectively clip hit for each model; therefore would not give extra
    # credit to reducdant information contained in the peer summary.
    if($scoreMode eq "A") {
  ...
  push(@$results,$gramScoreF);      # f1-measure score
  if($debug) {
    print "total O$NSIZE-gram model count: $totalGramCount\n";
    print "total O$NSIZE-gram peer count: $totalGramCountP\n";
    print "total O$NSIZE-gram hit: $totalGramHit\n";
    print "total OROUGE-$NSIZE\-R: $gramScore\n";
    print "total OROUGE-$NSIZE\-P: $gramScoreP\n";
    print "total OROUGE-$NSIZE\-F: $gramScoreF\n";
  }
}
...
sub ongramScore {
  ...
  foreach $t (@tokens) {
    if($t ne "_cn_" and $t ne "_word_index_") {
      my $h;
      $h=0;
      if(exists($peer_grams->{$t})) {
        ...
        #   DISTANCE CALCULATION  (START)                                                                        # 1.5.6
        my ($distance,$new_distance,@peer_indices,@model_indices,$j,$i);

#        print "token: $t\n";
#        print "peer indices: @{$peer_grams->{'_word_index_'}->{$t}}\n";
#        print "peer count: $peer_grams->{$t}\n";
#        print "model indices: @{$model_grams->{'_word_index_'}->{$t}}\n";
#        print "model count: $model_grams->{$t}\n";
        @peer_indices = @{$peer_grams->{"_word_index_"}->{$t}};
        @model_indices = @{$model_grams->{"_word_index_"}->{$t}};
        $distance = abs($peer_indices[0] - $model_indices[0]);
        $new_distance = $distance;
        foreach $i (@peer_indices) {
          foreach $j (@model_indices) {
            $new_distance = abs($i-$j);
            if ($new_distance < $distance) {
              $distance = $new_distance;
            }
          }
        }
        #   DISTANCE CALCULATION  (END  )                                                                        # 1.5.6

        $$hit+=($h * ($orderWeight ** $distance));
      }
    }
  }
